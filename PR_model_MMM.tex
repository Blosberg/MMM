...
\,Declining voter participation, organized strategic-voting campaigns, public opinion polls\cite{Broadbent_poll}, and myriad other signals highlight the need to change Canada's current first-past-the-post (FPP) system. It remains very much unclear, however, \emph{how} the system ought to be improved. \\
...
This method eliminates under-representation of parties, while preserving incentive for parties to win local races; it preserves the individual relationship between every voter and an MP, and it enables voters to distinguish between support for local candidates and national parties.  The model also avoids unnecessary `dilution' of constituency representatives; as such, it can be thought of as an optimization of mixed-member PR.
A key feature of this model is conservatism \--it modifies our existing system with the minimal set of changes necessary to resolve the most obvious problems in the current system, without introducing unnecessary changes. It fixes \emph{only} what is broken.
\end{abstract}
...
In a general election, however, there are various overlapping preferences that are impossible to disentangle with a single ballot mark.
Given the mechanics of Canada's FPP system, it would seem that the question being posed to voters is as follows:
