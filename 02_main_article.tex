...
Unlike traditional MM, however, it preserves the incentive for parties to win local races, it avoids unnecessary `dilution' of constituency representatives, and it avoids misattribution of spoiled ballots to major parties, by preserving voters' rights to vote for \emph{no party}.
As such, it can be thought of as an optimization of MM.
A key feature of this model is conservatism \--it modifies our existing FPP system with the minimal set of changes necessary to resolve its most obvious problems, without creating new ones.
