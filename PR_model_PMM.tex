...
\title{The Parsimonious Mixed-Member Model} % Your article title
...
It remains very much unclear, however, \emph{how} the system ought to be changed.\\
\,This paper briefly reviews some models for electoral reform (ER) from other nations, before putting forward the `parsimonious mixed-member' (PMM) model for consideration, which addresses the most common ER issues, enumerated here.\\
This model was inspired by the mixed-member proportional representation (PR) system currently used in Germany. However, it is `parsimonious' in the sense that the number of additional MPs brought into parliament to reach proportionality is minimized.
This method eliminates under-representation of parties, while preserving incentive for parties to win local races; it preserves the individual relationship between every voter and an MP, and it enables voters to distinguish between support for local candidates and national parties.  
The model also avoids unnecessary `dilution' of constituency representatives; as such, it can be thought of as an optimization of mixed-member PR.
A key feature of this model is conservatism \--it modifies our existing system with the minimal set of changes necessary to resolve the most obvious problems in the current system, without introducing unnecessary changes. 
...
Assessing the performance of any electoral system requires first clarifying what it is intended to count; this is more subtle than it first appears. 
Referenda, by contrast, establish a rather unambiguous level of support for options `yes' and `no'. 
In general elections, however, there are various overlapping preferences that are impossible to disentangle with a single ballot mark.
...
%----------------------------------------------------------------------------------------
%-------- COMMENTED OUT THIS STUFF... MIGHT BRING IT BACK IN LATER....--------------------
